...
a <- new %>% 
  tabyl(EducLevel, RaceAndEthnic) %>% 
  adorn_totals(where = c("row", "col")) %>% 
  adorn_percentages(denominator = "all") %>% 
  adorn_pct_formatting(digits = 1) %>% 
  as.data.frame()
a$EducLevel <- c("Under High School", "High School", "College", "Professional", "Master", "Doctor", "Total")
colnames(a) <- c("EducLevel", "White", "Black", "Asian", "Hispanic", "Total")
png("observedata.png")
p<-tableGrob(a)
grid.arrange(p)
dev.off()
